...
import { getUserRankedMovies } from "@/lib/server/stats/user-ranked-movies";
...
export default async function handler(
  ...
  try {
    const body = await getUserRankedMovies(cineforumId, userId);
    return res.status(200).json({ body, status: "completed" });
  } catch (error) {
    console.error(
      "Error in GET /api/cineforum/[cineforumId]/stats/users/ranked-movies:",
      error,
    );
